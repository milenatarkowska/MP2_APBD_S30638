...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie04_PierwszyPrzedmiotAnalityczny()
    {
        ...
        if (przedmiot == null)
        {
            return new List<string> { "Nie ma przedmiotu w tej kategorii" };
        }
    ...
    public IEnumerable<string> Zadanie05_CzyIstniejeNieaktywneZapisanie()
    {
        ...
        return new List<string>{result ? "True" : "False"};
    ...
    public IEnumerable<string> Zadanie06_CzyWszyscyProwadzacyMajaKatedre()
    {
        var result = DaneUczelni.Prowadzacy
            .All(prowadzacy => !prowadzacy.Katedra.Equals(null));
        return new List<string>{result ? "True" : "False"};
    ...
    {
        var active = DaneUczelni.Zapisy
            .Count(zapis => zapis.CzyAktywny);
        return new List<string>{active.ToString()};
    ...
    {
        var uniqueCities = DaneUczelni.Studenci
    ...
    public IEnumerable<string> Zadanie12_ParyStudentPrzedmiot()
    {
        var result = DaneUczelni.Zapisy
            ...
            .Join(DaneUczelni.Przedmioty,
                combined => combined.zapis.PrzedmiotId,
                przedmiot => przedmiot.Id,
                (combined, przedmiot) => $"{combined.student.Imie} {combined.student.Nazwisko} {przedmiot.Nazwa}");
        return result;
    ...
    {
        var result = DaneUczelni.Zapisy
            .Where(zapis => !zapis.OcenaKoncowa.Equals(null))
            .Join(DaneUczelni.Przedmioty,
                zapis => zapis.PrzedmiotId,
                przedmiot => przedmiot.Id,
                (zapis, przedmiot) => new { przedmiot.Nazwa, zapis.OcenaKoncowa })
            .GroupBy(res => res.Nazwa)
            .Select(group => $"{group.Key} {group.Average(x => x.OcenaKoncowa):F2}");
        return result;
    ...
    public IEnumerable<string> Zadanie15_ProwadzacyILiczbaPrzedmiotow()
    {
        var result = DaneUczelni.Prowadzacy
            .GroupJoin(DaneUczelni.Przedmioty,
                ...
                (prowadzacy, przedmiotyGrupa) => new
                {
                    prowadzacy.Imie,
                    prowadzacy.Nazwisko,
                    Liczba = przedmiotyGrupa.Count()
                })
            .Select(res => $"{res.Imie} {res.Nazwisko} {res.Liczba}");
        return result;
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        var result = DaneUczelni.Studenci
            ...
            .GroupBy(res => new { res.Imie, res.Nazwisko })
            .Select(group => $"{group.Key.Imie} {group.Key.Nazwisko} {group.Max(x => x.OcenaKoncowa)}");
        return result;
    ...
    {
        var result = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy.Where(z => z.CzyAktywny.Equals(true)),
                student => student.Id,
                zapis => zapis.StudentId,
                (student, zapisy) => new
                {
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        var result = DaneUczelni.Przedmioty
            ...
            .GroupBy(res => res.Nazwa)
            .Where(group => group.All(x => x.OcenaKoncowa == null))
        ...
        var result = DaneUczelni.Prowadzacy
            .GroupJoin(DaneUczelni.Przedmioty,
                ...
                (prowadzacy, przedmiotyGrupa) => new
                {
                    prowadzacy.Imie,
                    prowadzacy.Nazwisko,
                    PrzedmiotyId = przedmiotyGrupa.Select(x => x.Id)
                })
            .Select(x => new
            {
                x.Imie,
                x.Nazwisko,
                Avg = DaneUczelni.Zapisy
                    .Where(zapis => x.PrzedmiotyId.Contains(zapis.PrzedmiotId) && zapis.OcenaKoncowa != null)
                    .Average(z => z.OcenaKoncowa)
            })
            .Select(res =>
                $"{res.Imie} {res.Nazwisko} {(res.Avg.HasValue ? res.Avg.Value.ToString("F2") : "Brak ocen")}");
        return result;
    ...
    {
        var result = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy.Where(zapis => zapis.CzyAktywny.Equals(true)),
                student => student.Id,
                zapis => zapis.StudentId,
                (student, zapis) => new {student.Miasto})
            .GroupBy(res => res.Miasto)
            .Select(group => new
            {
                Miasto = group.Key,
                Liczba = group.Count()
            })
            .OrderByDescending(res => res.Liczba)
            .Select(res => $"{res.Miasto} {res.Liczba}");
        return result;
